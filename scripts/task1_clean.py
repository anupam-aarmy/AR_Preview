...
Clean implementation following AI Assignment requirements
"""
...
def place_product_on_wall(image, wall_mask, product_path):
    ...
    # Blend the product directly without shadows - PRESERVE ORIGINAL PRODUCT COLORS
    for c in range(3):
...
def main():
    """Main Task 1 pipeline"""
    print("🚀 Task 1: SAM Wall Segmentation + Product Placement")

    # Configuration
    room_image_path = "assets/room_wall.png"
    output_dir = "output/task1_deterministic"
    use_fast_mode = True
    ...
    try:
        ...
        # Segment walls once for this room
        ...
        for product_path, product_name in products:
            if not os.path.exists(product_path):
                print(f"⚠️ Skipping {product_path} - file not found")
                continue
